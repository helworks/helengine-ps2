...
namespace helengine::ps2 {
    namespace {
        ...
        constexpr double LightingScale = 124.15;
        ...
        constexpr bool EnableLegacyCpuTexturedOpaquePath = false;
        ...
        constexpr bool UseDirectGifTexturedSubmission = false;
        ...
        bool CanBuildBoundedUntexturedAggregatePacket(const Ps2VuOpaqueBatch& batch) {
            if (batch.Model == nullptr) {
                return false;
            }

            return (static_cast<std::size_t>(batch.Model->GetTriangleVertexCount()) / 3u)
        ...
        std::size_t BuildCompatibleUntexturedGroups(
            ...
            compatibleBatches.clear();
            if (firstBatchIndex >= batches.size()) {
                return firstBatchIndex;
            }
            ...
            while (batchIndex < batches.size()) {
                ...
                compatibleBatches.push_back(&candidate);
                batchIndex++;
            }
    ...
    ::RuntimeMaterial* Ps2RenderManager3D::BuildMaterialFromCooked(std::string cookedAssetPath, IContentStreamSource* contentStreamSource) {
    ...
    ::RuntimeModel* Ps2RenderManager3D::BuildModelFromCooked(std::string cookedAssetPath, IContentStreamSource* contentStreamSource) {
        (void)contentStreamSource;
        if (cookedAssetPath.empty()) {
    ...
    void Ps2RenderManager3D::RenderOpaqueWithVuPath(const Ps2FramePlan& plan, const ::float4x4& view, const ::float4x4& projection, const ::float4& viewport, float nearPlaneDistance) {
        ...
        std::vector<Ps2VuOpaqueBatchSlice> texturedVuBatches;
        ...
        std::vector<::float4x4> texturedVuWorlds;
        std::vector<GSTEXTURE*> texturedVuTextures;
        std::vector<int> texturedVuTextureWidths;
        std::vector<int> texturedVuTextureHeights;
        std::vector<Ps2VuOpaqueBatch> runtimeWhiteTexturedBatches;
        cpuFallbackTexturedBatches.reserve(batches.size());
        texturedVuBatches.reserve(batches.size());
        ...
        texturedVuWorlds.reserve(batches.size());
        texturedVuTextures.reserve(batches.size());
        texturedVuTextureWidths.reserve(batches.size());
        texturedVuTextureHeights.reserve(batches.size());
        ...
        for (std::size_t batchIndex = 0u; batchIndex < batches.size(); batchIndex++) {
            ...
            const bool canUseTexturedVuFastPath = CanUseTexturedVuFastPath(batch, world, view, projection, nearPlaneDistance);
            if (texturedBatchClassificationDiagnosticCount < TexturedVuSubmitDiagnosticLogLimit) {
                ...
                AppendDiagnosticToHostBootLog(
                    ...
                    + " vuFastPathEligible=" + std::to_string(canUseTexturedVuFastPath ? 1 : 0));
            }
                ...
                std::vector<Ps2VuOpaqueBatchSlice> batchSlices = CreateOpaqueBatchSlices(
                    *texturedBatch,
                    useTexturedVuSourcePath
                        ? MaximumTexturedVuSourceTriangleCount
                        : MaximumBoundedTexturedAggregateSourceTriangleCount);
                for (const Ps2VuOpaqueBatchSlice& batchSlice : batchSlices) {
                    if (useTexturedVuSourcePath) {
                        texturedVuBatches.push_back(batchSlice);
                        texturedVuWorlds.push_back(world);
            ...
            if (!EnableUntexturedAggregatePacketDiagnostics
                && !batch.Textured
                && CanBuildBoundedUntexturedAggregatePacket(batch)
                && !EnableVuDispatchBypassDiagnostics
                ...
                for (const Ps2VuOpaqueBatch* compatibleBatch : compatibleBatches) {
                    compatibleWorlds.push_back(BuildWorldMatrix(*compatibleBatch->Proxy));
                }

                std::size_t consumedBatchCount = 0u;
                while (consumedBatchCount < compatibleBatches.size()) {
                    std::vector<const Ps2VuOpaqueBatch*> packetBatches(
                        compatibleBatches.begin() + consumedBatchCount,
                        compatibleBatches.end());
                    std::vector<::float4x4> packetWorlds(
                        compatibleWorlds.begin() + consumedBatchCount,
                        compatibleWorlds.end());
                    VuGifStateEncoder.EncodeOpaqueState(*packetBatches.front(), GsGlobal);
                    VuVifPacketBuilder.Reset();
                    const std::clock_t vuPacketEncodeStartTicks = std::clock();
                    const std::size_t packetBatchCount = VuVifPacketBuilder.AddOpaqueUntexturedBatches(
                        packetBatches,
                        packetWorlds,
                        view,
                        projection,
        ...
        if (!texturedVuBatches.empty()) {
            std::size_t firstTexturedVuBatchIndex = 0u;
            const std::size_t maximumTexturedVuSourceBatchCount = Ps2VuVifPacketBuilder::GetMaximumTexturedVuSourceBatchCount();
            while (firstTexturedVuBatchIndex < texturedVuBatches.size()) {
                ...
                VuPacketSlots[ActiveVuPacketSlotIndex] = VuVifPacketBuilder.ReleasePacket();
                texturedVuPacket = VuPacketSlots[ActiveVuPacketSlotIndex];
                const std::clock_t vuSubmitStartTicks = std::clock();
                dma_channel_send_packet2(texturedVuPacket, DMA_CHANNEL_VIF1, 1);
                const std::clock_t vuSubmitEndTicks = std::clock();
                LastVuSubmitMilliseconds += ResolveMillisecondsFromClockTicks(vuSubmitStartTicks, vuSubmitEndTicks);
                LastPerformanceMetrics.VifSubmitMilliseconds += ResolveMillisecondsFromClockTicks(vuSubmitStartTicks, vuSubmitEndTicks);
                LastVuBatchDispatchCount += 1u;
    ...
    std::size_t Ps2RenderManager3D::GetLastUntexturedSubmittedTriangleCount() const {
        return 0u;
    }

    std::size_t Ps2RenderManager3D::GetLastTexturedPacketCacheBuildCount() const {
        return 0u;
    }

    std::size_t Ps2RenderManager3D::GetLastFrustumRejectedBatchCount() const {
        return 0u;
    }
    ...
    void Ps2RenderManager3D::SetLastVifDrainMilliseconds(double milliseconds) {
        if (milliseconds < 0.0) {
            throw std::invalid_argument("PS2 VIF drain duration cannot be negative.");
        }

        LastPerformanceMetrics.VifDrainMilliseconds = milliseconds;
    }
